...
static PSI_memory_info all_collector_memory[]=
{
  {&key_memory_mysql_heartbeat_context, "mysql_heartbeat_context", 0}
};

static void init_collector_psi_keys()
{
  const char* category= "collector";
  int count;

  count= array_elements(all_collector_memory);
  mysql_memory_register(category, all_collector_memory, count);
};
...
/*
  Initialize the collector  at server start or plugin installation.

  SYNOPSIS
    collector_plugin_init()
...
static int collector_plugin_init(void *p)
{
  DBUG_ENTER("collector_plugin_init");

#ifdef HAVE_PSI_INTERFACE
  init_collector_psi_keys();
#endif
...
/*
  Terminate the at server shutdown or plugin deinstallation.

  SYNOPSIS
    collector_plugin_deinit()
    Does nothing.
...
static int collector_plugin_deinit(void *p)
{
  DBUG_ENTER("collector_plugin_deinit");
  char buffer[HEART_STRING_BUFFER];
  struct st_plugin_int *plugin= (struct st_plugin_int *)p;
  struct mysql_heartbeat_context *con=
...
struct st_mysql_daemon collector_plugin=
...
mysql_declare_plugin(collector)
{
  MYSQL_DAEMON_PLUGIN,
  &collector_plugin,
  "collector",
  "tsthght",
  "collect environment and system interval information when something occurs.",
  PLUGIN_LICENSE_GPL,
  collector_plugin_init, /* Plugin Init */
  collector_plugin_deinit, /* Plugin Deinit */
  0x0100 /* 1.0 */,
